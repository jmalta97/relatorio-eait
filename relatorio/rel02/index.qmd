...
Conforme estabelecido pela @ABNT12655, o controle tecnológico envolve a verificação sistemática das propriedades
do concreto, desde sua dosagem até seu endurecimento, assegurando que o
material atenda aos requisitos de projeto.
...
A Tabela 10 apresenta a resistência média por idade do concreto, obtida com a função tapply():
...
Utilizando o pacote leem, foi gerado o gráfico de barras da resistência média por idade (Figura 4) com a função barplot():
...
- Cimento: O consumo de cimento é maior para classes mais altas, coerente com a necessidade de maior resistência (Mehta & Monteiro, 2014).
...
**inserir formula dda resistencia relativa que é R=resistencia_mpa/referencia de classe**

A resistência de referência foi definida com base no valor característico da classe de concreto (fck), conforme estabelecido pela NBR 6118 (ABNT, 2014). Assim, para a classe C25, a referência é 25 MPa; para C30, 30 MPa; para C35, 35 MPa; e para C40, 40 MPa.
...
# Verificar
table(dados_limpos$classificacao, useNA = "ifany")
...
barplot(table(dados_limpos$classificacao, useNA = "no"),
        ...
        ylim = c(0, 50))
```
A predominância de classificações "Excelente" e "Bom" (73%) indica que, em geral, os concretos estão atendendo às especificações. No entanto, os 4% classificados como "Insatisfatório" merecem atenção, pois representam corpos de prova com resistência significativamente abaixo do esperado para sua classe, conforme recomendado por Mehta & Monteiro (2014).
...
table(dados_limpos$acima_media, useNA = "ifany")
```
...
Discussão sobre interpretação física:
...
```{r}
table(acima_media_classe, useNA = "ifany")
```
...
table(dados_limpos$acima_media_classe_vec, useNA = "ifany")
```
...
kable(tabela_comparacao, 
      caption = "Tabela 2: Comparação entre for + if e vetorização em R",
      booktabs = TRUE,
...
kable(tabela_medias, 
      caption = "Tabela 3: Média de resistência à compressão por obra",
      booktabs = TRUE,
      align = c("l", "c")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE,
...
Neste tópico será analisada a afirmação:
...
O valor obtido foi de 0,732.

Análise: A correlação positiva forte (0,73) confirma a afirmação do engenheiro: "quanto maior o consumo de cimento, maior tende a ser a resistência do concreto". Esta relação é bem conhecida na tecnologia do concreto e é um dos fundamentos do método de dosagem.
...
Análise: A correlação negativa forte (-0,85) indica que quanto menor a relação água/cimento, maior a resistência à compressão, confirmando a Lei de Abrams. Este é um dos resultados mais importantes para o controle tecnológico do concreto.
...
Utilizando o pacote leem, foi gerado o boxplot da resistência por idade (Figura 7) com a função boxplot():
...
Discussão: O ganho de resistência com a idade é um fenômeno conhecido e esperado, resultante da continuação das reações de hidratação do cimento. Para concretos estruturais, a resistência de referência geralmente é a de 28 dias, conforme estabelecido pelas normas técnicas (NBR 6118, 2014).
...
A relação água/cimento confirmou ser o principal fator de controle da resistência do concreto, com correlação negativa forte (r = -0,85), consistente com a Lei de Abrams.

O consumo de cimento apresentou correlação positiva forte (r = 0,73) com a resistência, confirmando a intuição técnica.
...
Aproximadamente 73% dos corpos de prova foram classificados como "Excelente" ou "Bom" em relação à resistência esperada para sua classe, indicando um bom controle de qualidade geral.
