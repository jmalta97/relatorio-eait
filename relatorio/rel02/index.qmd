...
de qualidade de cada obra (Mehta & Monteiro, 2014).

Utilizando o pacote leem, foi gerado o boxplot da resistência por bloco
(Figura 2) com a função boxplot():
...
A Tabela 9 apresenta a resistência média por classe de concreto, obtida
com a função tapply():
...
como esperado pela especificação técnica (NBR 6118, 2014). A classe C40
...
A Tabela 10 apresenta a resistência média por idade do concreto, obtida
com a função tapply():
...
Utilizando o pacote leem, foi gerado o gráfico de barras da resistência
média por idade (Figura 4) com a função barplot():
...
- Resistência: Aumenta com a classe do concreto (C25 → C40), como
  esperado segundo Neville (2016).

- Cimento: O consumo de cimento é maior para classes mais altas,
  coerente com a necessidade de maior resistência (Mehta & Monteiro,
  2014).
...
**inserir formula dda resistencia relativa que é
R=resistencia_mpa/referencia de classe**
...
NBR 6118 (ABNT, 2014). Assim, para a classe C25, a referência é 25 MPa;
para C30, 30 MPa; para C35, 35 MPa; e para C40, 40 MPa.

A resistência relativa permite avaliar o desempenho de cada corpo de
prova em relação ao valor esperado para sua classe (Neville, 2016).
...
A predominância de classificações "Excelente" e "Bom" (73%) indica que,
em geral, os concretos estão atendendo às especificações. No entanto, os
4% classificados como "Insatisfatório" merecem atenção, pois representam
corpos de prova com resistência significativamente abaixo do esperado
para sua classe, conforme recomendado por Mehta & Monteiro (2014).
...
grandes assimetrias (Neville, 2016).
...
Análise: A correlação positiva forte (0,73) confirma a afirmação do
engenheiro: "quanto maior o consumo de cimento, maior tende a ser a
resistência do concreto". Esta relação é bem conhecida na tecnologia do
concreto e é um dos fundamentos do método de dosagem.
