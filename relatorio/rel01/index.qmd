...
A média aritmética é apresentada como uma das medidas mais conhecidas e utilizadas. Ela representa um ponto de equilíbrio dos dados e pode ser calculada tanto para dados não agrupados quanto para dados agrupados. No entanto, o capítulo destaca que a média é sensível a valores discrepantes, ou seja, valores muito distantes da maioria das observações podem alterar significativamente seu resultado. Para esses casos sugere-se a utilização da média aparada, onde é excluído do cálculo o valor mínimo e máximo de um conjunto de dados. 
**Inserir fórmula da média da população e da amostra aqui e da média aparada.** 

A mediana é definida como o valor central de um conjunto de dados ordenado. Ela divide o conjunto em duas partes, de modo que aproximadamente 50% dos valores fiquem abaixo e 50% fiquem acima. Por considerar a posição dos dados e não diretamente seus valores extremos, a mediana é menos influenciada por dados discrepantes/extremos. Quando há dados agrupados em intervalos de classes o cálculo utiliza outros parâmetros, como limite inferior e superior da classe, frequência acumulada da classe anterior à classe em questão e frequência absoluta da classe mediana. 

**Inserir fórmula da mediana da população e da amostra**
...
Os resultados obtidos podem ser observados na **tabela xx**
...
O grupo A apresentou resistência média de **28,166 MPa e mediana de 28,190 MPa.** A proximidade entre essas duas medidas é um primeiro indício de que a distribuição é equilibrada e não está sendo fortemente afetada por valores extremos.
...
O desvio padrão foi de **1,428 MPa**, indicando o afastamento típico dos resultados em torno da média. A amplitude foi de **4,580 MPa**, correspondente à diferença entre o maior resultado, **30,550 MPa, e o menor, 25,970 MPa.**

O coeficiente de variação foi:

**CV=1,428/28,166×100=5,07%**

Isso significa que o desvio padrão corresponde a aproximadamente **5,07% da média.** O grupo apresenta, portanto, uma variabilidade relativamente pequena em comparação com seu nível médio de resistência.

O excesso de curtose foi de **-1,018.** O resultado negativo sugere que, nessa amostra, os valores se apresentam de forma relativamente mais distribuída e com caudas mais leves do que a distribuição normal. Não foram identificados valores atípicos pelo critério do intervalo interquartil.

Em suma, o grupo A apresentou resistência média de **28,166 MPa**, baixa variabilidade relativa, distribuição aproximadamente simétrica e ausência de indícios estatísticos de valores discrepantes. A média e a mediana são praticamente iguais, indicando que ambas representam adequadamente o centro dos resultados.

#### Grupo B

O grupo B apresentou resistência média de **32,456 MPa e mediana de 32,475 MPa.** Novamente, observa-se grande proximidade entre as duas medidas, indicando que a média não está sendo fortemente deslocada por valores extremos.

O coeficiente de assimetria foi:

**g_1=0,291**

O sinal positivo indica uma leve tendência de cauda à direita, causada pelos resultados mais elevados, como **35,670 MPa e 36,050 MPa. Entretanto, o valor de 0,291** ainda é relativamente próximo de zero. Portanto, a assimetria pode ser considerada pequena.
...
**A=36,050-29,560=6,490" MPa"**

Esses resultados indicam que o grupo B possui maior dispersão absoluta. O coeficiente de variação foi de 5,84%, também o maior entre os três grupos. Embora a diferença não seja muito elevada, o grupo B é relativamente o menos homogêneo.

O excesso de curtose foi de-0,659, indicando uma distribuição relativamente achatada e sem forte concentração de resultados extremos. Pelo critério do intervalo interquartil, também não foram identificados valores atípicos.

Em suma, o grupo B apresentou resistência média de 32,456 MPa e leve assimetria positiva. Embora não tenham sido identificados valores discrepantes, o grupo apresentou a maior amplitude, o maior desvio padrão e o maior coeficiente de variação, sendo o grupo com maior variabilidade entre os corpos de prova.
...
Sua mediana foi de **39,670 MPa**, novamente muito próxima da média. Essa proximidade indica que a distribuição é equilibrada e que os valores altos ou baixos não estão deslocando significativamente a média.
O coeficiente de assimetria foi:
...
O desvio padrão foi de **1,660 MPa**, valor intermediário entre os grupos A e B. A amplitude foi:

**A=42,300-36,890=5,410" MPa"**
...
O excesso de curtose foi de **-0,801**, apontando uma distribuição relativamente mais achatada e sem caudas pesadas. Não foram identificados valores atípicos pelo critério do intervalo interquartil.
